...
let Incident = (_dec = NearBindgen({}), _dec2 = call({
  payableFunction: true
}), _dec3 = view(), _dec4 = view(), _dec(_class = (_class2 = class Incident {
...
function total_messages() {
  const _state = Incident._getState();
  if (!_state && Incident._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = Incident._create();
  if (_state) {
    Incident._reconstruct(_contract, _state);
  }
  const _args = Incident._getArgs();
  const _result = _contract.total_messages(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Incident._serialize(_result, true));
}
...
function add_message() {
  ...
  Incident._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Incident._serialize(_result, true));
}
